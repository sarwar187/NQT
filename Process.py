...
def read_data(opt):
    ...
    if opt.trg_data_retrieval is not None:
        try:
            opt.trg_data_retrieval = open(opt.trg_data_retrieval).read().strip().split('\n')
        except:
            print("error: '" + opt.trg_data_retrieval + "' file not found")
            quit()
...
def create_dataset(opt, SRC, TRG):
    ...
    #only build the src vocabulary here
    #creating
...
def create_retrieval_vocabulary(opt, TRG):
    print("creating target vocabulary ... ")
    raw_data = {'trg': [line for line in opt.trg_data] + [line for line in opt.trg_data_retrieval]}
    ...
    TRG.build_vocab(train_retrieval)
    opt.trg_pad = TRG.vocab.stoi['<pad>']

    # create another training file where there are two copies of CLEF as source and target
    # Treat that as just another dataset and use the target part only to build the vocabulary
    # The other way would be to push the terms in the vocabulary
