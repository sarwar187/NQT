...
def query_expansion(query_strings_list_all, inmem):
    ...
    size_array = [int(50) for i in np.arange(length)]
    if remainder!=0:
    ...
    for i in np.arange(len(size_array)):
        ...
        #inmem.output.close()
    return sorted(query_expansion_strings_tuples_all, key=lambda x: x[0])
...
def train_model(model, opt, SRC, TRG):
    ...
    for epoch in range(opt.epochs):
        ...
        for i, batch in enumerate(opt.train):
            start = time.clock()
            # ' '.join([TRG.vocab.itos[tok] for tok in outputs[0][1:length]])
            #print(batch)
            #print(type(batch))
            ...
            #print(trg_strings)
            print("trg strings size " + str(len(trg_strings)))
            #trg_strings_relevance_model = query_expansion(trg_strings, inmem)
            #print(trg_strings_relevance_model)
            #print("expansion strings size " + str(len(trg_strings_relevance_model)))
...
def main():
    ...
    parser.add_argument('-trg_data_retrieval', required=True)
